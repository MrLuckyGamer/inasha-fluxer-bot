...
interface GuildFamilyData {
  __proposals: Record<string, Proposal>;
  [userId: string]: FamilyMember | Record<string, Proposal>;
}

type FamilyData = Record<string, GuildFamilyData>;

function load(): FamilyData {
  return JSON.parse(fs.readFileSync(FILE, 'utf8'));
}
function save(data: FamilyData): void {
  fs.writeFileSync(FILE, JSON.stringify(data, null, 2));
}

function ensureGuild(data: FamilyData, guildId: string): void {
  if (!data[guildId]) data[guildId] = { __proposals: {} };
  if (!data[guildId].__proposals) data[guildId].__proposals = {};
}

function ensureUser(data: FamilyData, guildId: string, userId: string): void {
  ensureGuild(data, guildId);
  const guildData = data[guildId] as Record<string, FamilyMember>;
  if (!guildData[userId]) {
    guildData[userId] = { parents: [], children: [], spouse: null };
  } else if (guildData[userId].spouse === undefined) {
    guildData[userId].spouse = null;
  }
}
...
function getUser(data: FamilyData, guildId: string, userId: string): FamilyMember {
  const guildData = data[guildId] as Record<string, FamilyMember> | undefined;
  return guildData?.[userId] ?? { parents: [], children: [], spouse: null };
}

function getSiblings(data: FamilyData, guildId: string, userId: string): string[] {
  ...
  return Object.entries(data[guildId])
    .filter(
      ([uid, info]) =>
        uid !== '__proposals' &&
        uid !== userId &&
        (info as FamilyMember).parents?.some((p) => family.parents.includes(p))
    )
    .map(([uid]) => uid);
}

function getAncestors(data: FamilyData, guildId: string, userId: string, maxDepth = 10): Set<string> {
...
function clearExpiredProposals(data: FamilyData, guildId: string): void {
...
function embed(title: string): EmbedBuilder {
...
function buildTreeFields(data: FamilyData, guildId: string, userId: string, displayName: string): EmbedBuilder {
  const family = getUser(data, guildId, userId);
...
function buildBigTree(data: FamilyData, guildId: string, userId: string, displayName: string): string {
  const family = getUser(data, guildId, userId);
  const lines: string[] = [];
  ...
  lines.push(
    `${family.parents.length ? family.parents.map(mention).join(' & ') + '\n  └─ ' : ''}${
...
const command: Command = {
  ...
  async execute(message: Message, args: string[], client: Client) {
    ...
    const send = (e: EmbedBuilder) => sendToChannel(message, { embeds: [e] });
    const fail = (desc: string) => send(errorEmbed(desc));

    // ---- i>family help ----
    if (sub === 'help') {
      const helpEmbed = embed('👪 Family Command Help').setDescription(
        [
    ...
    if (sub === 'tree') {
      ...
      save(data);
      return send(
        embed(`🌳 ${targetName}'s Extended Family Tree`).setDescription(buildBigTree(data, guildId, targetId, targetName))
      );
    }
    ...
    if (sub === 'marry' || sub === 'adopt') {
      ...
      return send(
        embed(sub === 'marry' ? '💍 Marriage Proposal' : '📜 Adoption Request').setDescription(
