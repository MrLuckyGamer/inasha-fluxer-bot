  ...
  async execute(message: Message, args: string[], client: Client) {
    ...
    if (!targetMember?.roles.has(role.id))
      return sendToChannel(message, {
        embeds: [new EmbedBuilder().setColor('#FFA500').setDescription(`${target.username} doesn't have the **${role.name}** role!`)],
      });
    ...
    try {
      ...
      await sendToChannel(message, {
        embeds: [
          new EmbedBuilder()
            .setColor('#57F287')
            .setDescription(`Successfully removed **${role.name}** from ${target.username}!`)
