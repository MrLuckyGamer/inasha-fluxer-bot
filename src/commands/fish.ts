...
function ensureDir(f: string): void {
...
type FishCooldowns = Record<string, Record<string, number>>;

let fishData: FishScores = JSON.parse(fs.readFileSync(fishFile, 'utf8'));
let cooldowns: FishCooldowns = JSON.parse(fs.readFileSync(cooldownFile, 'utf8'));
...
function getRandomFish(): FishSpecies {
