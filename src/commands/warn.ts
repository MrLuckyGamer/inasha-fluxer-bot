...
type WarnStore = Record<string, Record<string, WarnEntry[]>>;

function loadWarns(): WarnStore {
  return JSON.parse(fs.readFileSync(WARN_FILE, 'utf8'));
}

function saveWarns(data: WarnStore): void {
  fs.writeFileSync(WARN_FILE, JSON.stringify(data, null, 2));
}

function renderDate(d: number | string | undefined): string {
...
const command: Command = {
  ...
  async execute(message: Message, args: string[], client: Client) {
    ...
    if (sub === 'view') {
      ...
      return sendToChannel(message, { embeds: [embed] });
    }
