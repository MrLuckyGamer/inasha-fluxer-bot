...
import { ChannelType, PermissionFlags, type Client, type Guild, type GuildChannel } from '@fluxerjs/core';
...
type ServerStatsStore = Record<string, ServerStatsConfig>;

function loadStats(): ServerStatsStore {
  return JSON.parse(fs.readFileSync(FILE, 'utf8'));
}
function saveStats(data: ServerStatsStore): void {
  fs.writeFileSync(FILE, JSON.stringify(data, null, 2));
}

// ── Update stat channel names ─────────────────────────────────────────────────
export async function updateStats(guild: Guild, client: Client): Promise<void> {
  ...
  try {
    ...
    const rename = async (id: string, name: string) => {
      const ch = (guild.channels.get(id) ?? client.channels.get(id)) as GuildChannel | undefined;
...
const command: Command = {
  ...
  async execute(message, args, client) {
    ...
    if (sub === 'enable') {
      ...
      const makeVoice = async (name: string) =>
        guild.createChannel({
          type: 2,
          name,
    ...
    if (sub === 'disable') {
      ...
      for (const key of ['users', 'bots', 'channels', 'category'] as const) {
        const ch = client.channels.get(cfg[key]) as GuildChannel | undefined;
